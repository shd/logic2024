...
\item Аргумент $M$ --- функция $f: (\alpha\rightarrow\bot)\rightarrow\bot$. Функция $f$, видимо (?), по аргументу $g : \alpha\rightarrow\bot$ 
угадывает $x : \alpha$ и выполняет вызов $g\ x$.

\item Выражение $M$ должно по функции $f$ как-то угадывать значение $x$. Необходима отдельная конструкция с отдельной семантикой.

\item Однако, похожие конструкции в языках бывают. Например, закон Пирса --- как тип для \verb!call/cc! из Scheme.
\end{itemize}
\end{frame}
