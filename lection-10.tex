...
любой $\alpha(x)$. Тогда $D$ представима в Ф.А.
\end{lmm}
...
Пусть $\alpha(x) := \forall p.\delta(x,p)\rightarrow\neg\sigma(p)$. Верно ли, что $\ulcorner\alpha(\overline{\ulcorner\alpha\urcorner})\urcorner\in\text{Th}_\mathcal{S}$?
\begin{itemize}
\item Если $\vdash_\mathcal{S}\alpha(\overline{\ulcorner\alpha\urcorner})$, то $\vdash_\mathcal{S}\forall p.\delta(\overline{\ulcorner\alpha\urcorner},p)\rightarrow\neg\sigma(p)$,
то есть $\vdash_\mathcal{S}\neg\sigma(\overline{\ulcorner\alpha(\overline{\ulcorner\alpha\urcorner})\urcorner})$

\item Если $\not\vdash_\mathcal{S}\alpha(\overline{\ulcorner\alpha\urcorner})$, то %при всех $p$ выполнено $\llbracket\sigma(p)\rrbracket=\text{Л}$, то есть
по выразимости $\vdash_\mathcal{S}\neg\sigma(\overline{\ulcorner\alpha(\overline{\ulcorner\alpha\urcorner})\urcorner})$,
отчего $\vdash_\mathcal{S}\alpha(\overline{\ulcorner\alpha\urcorner})$
\end{itemize}
\end{proof}
\end{frame}
